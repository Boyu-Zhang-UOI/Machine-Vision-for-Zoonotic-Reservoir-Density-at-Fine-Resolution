...
pairwise_model <- ranked |>
  ...
  stat_spike(data = ~.x |> filter(model_name == "Random"),
             lwd = 0.75,
             alpha = 1,
             height = 0.15,
             col = "dimgrey",
  ...
  stat_spike(data = ~.x |> filter(model_name == "Buildings"),
             lwd = 0.75,
             alpha = 1,
             height = 0.12,
             col = "dimgrey",
  ...
  stat_spike(data = ~.x |> filter(model_name == "BRT"),
             lwd = 0.75,
             alpha = 1,
             height = 0.05,
             col = "dimgrey",
...
pairwise_house <- ranked |>
  filter(house_name != "All") |>
  group_by(house_name, model_name) |>
  mutate(mean = mean(prob, na.rm = T)) |>
  dplyr::select(test_site, mean, house_name, model_name, prob) |>
  distinct() |>
...
ggsave("Figures/figure_5.png", ggarrange(pairwise_model, pairwise_house, widths = c(0.3, 0.7), label.y = 0.16, label.x = c(0.14, 0.13), labels = c("A","B")), width = 10, height = 6)
